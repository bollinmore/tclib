...
function() {
	...
	function(a, b) {
		...
		a.addBack = function(b) {
			...
			if($("nav.menu").hasClass("active")) {
			...
			} else if(ws.id == "index.html" || ws == plus.webview.getLaunchWebview()) ///增加首頁即退出程序
				plus.runtime.quit();
			else {
				...
				if(ws.id == "card_barcode.html")
					mui.fire(ws.opener().opener(), "init");
				else
					mui.fire(ws.opener(), "init");
		...
		a.os.plus && a.os.android && a.addBack({
			...
			handle: function() {
				...
				return d ? d.evalJS("mui&&mui.back();") : c.canBack(function(d) {
					d.canBack ? b.history.back() : c.id === plus.runtime.appid ? a.__back__first ? (new Date).getTime() - a.__back__first < 2e3 && plus.runtime.quit() : (a.__back__first = (new Date).getTime(), mui.toast("再按一次退出應用"), setTimeout(function() {
	...
	function(a, b, c) {
		...
		a.ajax = function(d, e) {
			...
			return c.add = function(a, b) {
				try {
					if(encodeURIComponent(a).indexOf("func") > -1) {

						var param = 'func=' + encodeURIComponent(b);
						var sign = "";
						var KeyNum = "";

						if(plus.os.name == 'Android') {
							var APISecurity = plus.android.importClass("com.claridy.util.security.APISecurity");
							sign = APISecurity.encrypt(param);
							KeyNum = APISecurity.getKeyNum();
						} else {
							var APISecurity = plus.ios.newObject("APISecurity");
							sign = plus.ios.invoke(APISecurity, "encrypt:", param);
							KeyNum = plus.ios.invoke(APISecurity, "getKeyNum");
							plus.ios.deleteObject(APISecurity);
						}

						this.push(encodeURIComponent("key") + "=" + encodeURIComponent(KeyNum));
						this.push(encodeURIComponent("sign") + "=" + encodeURIComponent(sign));
					}
				} catch(err) {}
			...
			var c = "mui-plus-pullrefresh",
				...
				k = a.Class.extend({
					...
					_init: function() {
						...
						window.addEventListener("dragup", a), b.addEventListener("plusscrollbottom", a),
							$("#pullrefresh").scroll(function(event) {
								!a.loading && ($("#pullrefresh").scrollTop() + $("#pullrefresh").innerHeight() + 40 >= $('#pullrefresh').prop('scrollHeight')) && (a.isScroll = !1, a.bottomPocket && a.pullupLoading())
							}),
							$(window).scroll(function(event) {
								!a.loading && (window.pageYOffset + window.innerHeight + 40 >= document.documentElement.scrollHeight) && (a.isScroll = !1, a.bottomPocket && a.pullupLoading())
							})
						...
						//
						////console.log("scroll"+($("#pullrefresh").scrollTop() + $("#pullrefresh").innerHeight() + 10));
						////
						////console.log("scrollHeight"+$('#pullrefresh').prop('scrollHeight'));
						//// console.log(($("#pullrefresh").scrollTop() + $("#pullrefresh").innerHeight() + 40 >= $('#pullrefresh').prop('scrollHeight') ));
						//// console.log(a.isScroll);
						//// console.log(a.loading);
						//// console.log(a.bottomPocket);
						//a.isScroll &&!a.loading && ($("#pullrefresh").scrollTop() + $("#pullrefresh").innerHeight() + 40 >= $('#pullrefresh').prop('scrollHeight') )&& (a.isScroll = !1, a.bottomPocket && a.pullupLoading())
						//						
						//						}, 500)
					},
					...
					scrollToBottom: function(c) {
						a.scrollTo($('#pullrefresh').prop('scrollHeight'), c)
					},
					refresh: function(a) {
	...
	function(a, b) {
		a.alert = function(c, d, e, f) {
			if(a.os.plus) {
				...
				"function" == typeof d ? (f = d, d = null, e = "確定") : "function" == typeof e && (f = e, e = null), a.plusReady(function() {
	...
	function(a, b, c) {
		var d = "mui-popup",
			...
			v = function(b, c, d, e, f) {
				return "undefined" != typeof b ? ("function" == typeof c ? (e = c, f = d, c = null, d = null) : "function" == typeof d && (f = e, e = d, d = null), a.os.plus && "div" !== f ? plus.nativeUI.alert(b, e, c || "提示", d || "確定") : u(s(b, c || "提示") + t([d || "確定"]), e)) : void 0
			},
			w = function(b, c, d, e, f) {
				return "undefined" != typeof b ? ("function" == typeof c ? (e = c, f = d, c = null, d = null) : "function" == typeof d && (f = e, e = d, d = null), a.os.plus && "div" !== f ? plus.nativeUI.confirm(b, e, c, d || ["取消", "確認"]) : u(s(b, c || "提示") + t(d || ["取消", "確認"]), e)) : void 0
			},
			x = function(b, c, d, e, f, g) {
				return "undefined" != typeof b ? ("function" == typeof c ? (f = c, g = d, c = null, d = null, e = null) : "function" == typeof d ? (f = d, g = e, d = null, e = null) : "function" == typeof e && (g = f, f = e, e = null), a.os.plus && "div" !== g ? plus.nativeUI.prompt(b, f, d || "提示", c, e || ["取消", "確認"]) : u(s(b, d || "提示", r(c)) + t(e || ["取消", "確認"]), f)) : void 0
			},
			y = function() {
	...
	function(a, b) {
		var c = /^rgba\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/,
			...
			e = function(b, c) {
				...
				if(!e.length) throw new Error("元素背景顏色必須為RGBA");
