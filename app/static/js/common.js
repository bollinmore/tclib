...
function copyToClip(bookName) {
	var Context = plus.android.importClass("android.content.Context");
	var main = plus.android.runtimeMainActivity();
	var clip = main.getSystemService(Context.CLIPBOARD_SERVICE);
	plus.android.invoke(clip, "setText", bookName);
}

function copyToClip_ios() {
...
(function(w) {
	...
	function plusReady() {
		...
		//
		//			if(ws.id == "index.html"||ws==plus.webview.getLaunchWebview()) ///增加首頁即退出程序
		//				plus.runtime.quit();
		//			else {
		//				if(typeof StopBeaconScanner_forbackmui == 'function') {
		//					StopBeaconScanner_forbackmui();
		//				}
		//				if(typeof StartBeaconScanner_forbackevent == 'function') {
		//					StartBeaconScanner_forbackevent();
		//				}
	 ...
	 */
	var isclickeding = false;
	w.clicked = function(id, wa, ns, ws) {
		if(plus.webview.currentWebview().id == id) { //避免可以多次打開當前頁面
			return null;
		}
		...
		if(w.plus) {
			var currentwev = plus.webview.currentWebview()
			if(plus.os.name == "Android") {
				...
				if(id == "index.html") {
					$.each(plus.webview.all(), function(i, web) {
						if(web != currentwev && web != plus.webview.getLaunchWebview())
							web.close("none");
					});
					currentwev.close("auto");
					mui.fire(plus.webview.getLaunchWebview(), "init");

				} else if(plus.webview.getWebviewById(id)) {
				...
				if(id != "index.html") {
					...
					ns || openw.addEventListener('loaded', function() {
						openw.show(as, 300, function() {
							isclickeding = false;
							if(currentwev.id == "card_barcode.html" ||(currentwev.id == "login.html" && id=="shelf_library.html"))
								currentwev.close("none");
						});
					}, false);
				...
				if(id == "index.html") {
					...
					var ws2 = plus.webview.currentWebview();
					ws2.draw(bitmap1, function() {
						wa && (waiting = plus.nativeUI.showWaiting());
						ws = ws || {};
						ws.scrollIndicator || (ws.scrollIndicator = 'none');
						ws.scalable || (ws.scalable = false);
