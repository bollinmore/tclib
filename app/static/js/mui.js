...
var mui = (function(document, undefined) {
	...
	/**
	 * mui isWindow(需考慮obj為undefined的情況)
	 */
	...
	/**
	 * 將 fn 緩存一段時間後, 再被調用執行
	 * 此方法為了避免在 ms 段時間內, 執行 fn 多次. 常用於 resize , scroll , mousemove 等連續性事件中;
	 * 當 ms 設置為 -1, 表示立即執行 fn, 即和直接調用 fn 一樣;
	 * 調用返回函數的 stop 停止最後一次的 buffer 效果
	 * @param {Object} fn
	 * @param {Object} ms
	...
	$.buffer = function(fn, ms, context) {
		...
		return $.extend(function() {
			if (
				(!lastStart) || // 從未運行過
				(lastEnd >= lastStart && $.now() - lastEnd > ms) || // 上次運行成功後已經超過ms毫秒
				(lastEnd < lastStart && $.now() - lastStart > ms * 8) // 上次運行或未完成，後8*ms毫秒
			) {
				run();
			} else {
				if (timer) {
...
(function($, document) {
	function detect(ua) {
		...
		if (plus) {
			...
			if (ua.match(/StreamApp/i)) { //TODO 最好有流應用自己的標識
				this.os.stream = true;
				$(function() {
...
/**
 * 僅提供簡單的on，off(僅支持事件委托，不支持當前元素綁定，當前元素綁定請直接使用addEventListener,removeEventListener)
 * @param {Object} $
 */
(function($) {
	 ...
	 * @returns {undefined}
	 */
	$.fn.on = function(event, selector, callback) { //僅支持簡單的事件委托,主要是tap事件使用，類似mouse,focus之類暫不封裝支持
	...
	$.fn.off = function(event, selector, callback) {
		return this.each(function() {
			...
			if (delegates[_mid]) {
				//如果off掉了所有當前element的指定的event事件，則remove掉當前element的delegate回調
...
(function($, window, document) {
	...
	window.addEventListener('click', function(event) { //解決touch與click的target不一致的問題(比如鏈接邊緣點擊時，touch的target為html，而click的target為A)
		var target = event.target;
		var isFound = false;
		for (; target && target !== document; target = target.parentNode) {
			if (target.tagName === 'A') {
...
(function($, window, name) {
	if (!$.os.android && !$.os.ios) { //目前僅識別android和ios
		return;
	}
	if (window.FastClick) {
		return;
...
(function($, window) {
	...
	var getTouches = function(event, touch) {
		...
		//當touchstart或touchmove且touches長度為1，直接獲得all和changed
	...
	window.addEventListener($.EVENT_CLICK, function(e) {
		//TODO 應該判斷當前target是不是在targets.popover內部，而不是非要相等
	...
	var handle = function(event, touch) {
		...
		switch (event.type) {
			...
			case $.EVENT_MOVE:
				...
				//修正direction,可在session期間自行鎖定拖拽方向，方便開發scroll類不同方向拖拽插件嵌套
		...
		switch (event.type) {
			...
			case $.EVENT_MOVE:
				if ($.options.gestureConfig.pinch) {
					...
					var scaleDiff = 0.000000000001; //防止scale與lastScale相等，不觸發事件的情況。
...
(function($) {
	...
	//TODO 自動調用init?因為用戶自己調用init的時機可能不確定，如果晚於自動init，則會有潛在問題
	//	$.ready(function() {
	//		setTimeout(function() {
	//			if (!isInitialized) {
	//				$.init();
	...
	$.addInit({
		...
		handle: function() {
			...
			if (!$.os.plus && subpages.length) {
				//暫時隻處理單個subpage。後續可以考慮支持多個subpage
...
(function($) {
	var defaultOptions = {
		...
		preloadLimit: 10, //預加載窗口的數量限製(一旦超出，先進先出)
		keyEventBind: {
			backbutton: true,
			menubutton: true
		}
	...
	$.plusReady = function(callback) {
		if (window.plus) {
			setTimeout(function() { //解決callback與plusready事件的執行時機問題(典型案例:showWaiting,closeWaiting)
				callback();
			}, 0);
		} else {
	...
	/**
	 * 打開新窗口
	 * @param {string} url 要打開的頁麵地址
	 * @param {string} id 指定頁麵ID
	 * @param {object} options 可選:參數,等待,窗口,顯示配置{params:{},waiting:{},styles:{},show:{}}
	 */
	$.openWindow = function(url, id, options) {
		...
		if (!$.os.plus) {
			//TODO 先臨時這麼處理：手機上頂層跳，PC上parent跳
		...
		if (webviewCache && webview) { //已緩存
			//每次show都需要傳遞動畫參數；
			//預加載的動畫參數優先級：openWindow配置>preloadPages配置>mui默認配置；
			...
			if (nShow.autoShow) {
				...
				//TODO 能走到這一步，應該不用判斷url了吧？
				if (!url) {
					showWebview();
				} else {
					// webview.addEventListener("loaded", showWebview, false);
					//titleUpdate觸發時機早於loaded，更換為titleUpdate後，可以更早的顯示webview
					webview.addEventListener("titleUpdate", showWebview, false);
					//loaded事件發生後，觸發預加載和pagebeforeshow事件
	...
	$.createWindow = function(options, isCreate) {
		...
		var webview;
		if (options.preload) {
			if ($.webviews[id] && $.webviews[id].webview.getURL()) { //已經cache
				webview = $.webviews[id].webview;
			} else { //新增預加載窗口
				//preload
				//判斷是否攜帶createNew參數，默認為false
				if (options.createNew !== true) {
					webview = plus.webview.getWebviewById(id);
				}
					...
					if (options.subpages) {
						$.each(options.subpages, function(index, subpage) {
							...
							if (subpageId) { //過濾空對象
								...
								if (!subWebview) { //如果該webview不存在，則創建
			...
			//TODO 理論上，子webview也應該計算到預加載隊列中，但這樣就麻煩了，要退必須退整體，否則可能出現問題；
			$.webviews[id] = {
				webview: webview, //目前僅preload的緩存webview
				preload: true,
				show: $.showOptions(options.show),
				afterShowMethodName: options.afterShowMethodName //就不應該用evalJS。應該是通過事件消息通訊
			};
			//索引該預加載窗口
			var preloads = $.data.preloads;
			var index = preloads.indexOf(id);
			if (~index) { //刪除已存在的(變相調整插入位置)
	...
	$.preload = function(options) {
		//調用預加載函數，不管是否傳遞preload參數，強製變為true
		if (!options.preload) {
		...
		if (opened) {
			for (var i = 0, len = opened.length; i < len; i++) {
				...
				if (open_open && open_open.length > 0) {
					...
					//如果直接孩子節點，就不用關閉了，因為父關閉的時候，會自動關閉子；
	...
	$.appendWebview = function(options) {
		...
		var id = options.id || options.url;
		var webview;
		if (!$.webviews[id]) { //保證執行一遍
			//TODO 這裏也有隱患，比如某個webview不是作為subpage創建的，而是作為target webview的話；
			if (!plus.webview.getWebviewById(id)) {
				webview = plus.webview.create(options.url, id, options.styles, options.extras);
			}
			//之前的實現方案：子窗口loaded之後再append到父窗口中；
			//問題：部分子窗口loaded事件發生較晚，此時執行父窗口的children方法會返回空，導致父子通訊失敗；
			//     比如父頁麵執行完preload事件後，需觸發子頁麵的preload事件，此時未append的話，就無法觸發；
			//修改方式：不再監控loaded事件，直接append
			//by chb@20150521
			// webview.addEventListener('loaded', function() {
	...
	$.addInit({
		...
		handle: function() {
			...
			if ($.os.plus) {
				$.plusReady(function() {
					//TODO  這裏需要判斷一下，最好等子窗口加載完畢後，再調用主窗口的show方法；
					//或者：在openwindow方法中，監聽實現；
					...
					if (plus.webview.currentWebview() === plus.webview.getWebviewById(plus.runtime.appid)) {
						$.isHomePage = true;
						//首頁需要自己激活預加載；
						//timeout因為子頁麵loaded之後才append的，防止子頁麵尚未append、從而導致其preload未觸發的問題；
						setTimeout(function() {
					...
					if ($.os.android && parseFloat($.os.version) < 4.4) {
						//解決Android平台4.4版本以下，resume後，父窗體標題延遲渲染的問題；
				...
				//					span.innerHTML = '在該瀏覽器下，不支持創建子頁麵，具體參考';
				//					err.appendChild(span);
				//					var a = document.createElement('a');
				//					a.innerHTML = '"mui框架適用場景"';
				//					a.href = 'http://ask.dcloud.net.cn/article/113';
				//					err.appendChild(a);
				//					document.body.appendChild(err);
				//					console.log('在該瀏覽器下，不支持創建子頁麵');
				//				}
...
(function($, window) {
		...
		handle: function() {
			...
			if (parent) {
				...
				wobj.canBack(function(e) {
					//by chb 暫時注釋，在碰到類似popover之類的錨點的時候，需多次點擊才能返回；
					if (e.canBack) { //webview history back
						window.history.back();
					} else { //webview close or hide
						//fixed by fxy 此處不應該用opener判斷，因為用戶有可能自己close掉當前窗口的opener。這樣的話。opener就為空了，導致不能執行close
						if (wobj.id === plus.runtime.appid) { //首頁
							//首頁不存在opener的情況下，後退實際上應該是退出應用；
							//首次按鍵，提示‘再按一次退出應用’
							if (!$.__back__first) {
	...
	$.menu = function() {
		...
		if (menu) {
			$.trigger(menu, $.EVENT_START); //臨時處理menu無touchstart的話，找不到當前targets的問題
			$.trigger(menu, 'tap');
		} else { //執行父窗口的menu
			if (window.plus) {
						...
						if ($.os.plus && $.os.android) { //android 5+
							$.plusReady(function() {
								...
								if (hasPulldown) {
									...
									if (parent) {
										if (!hasPullup) { //如果沒有上拉加載，需要手動初始化一個默認的pullRefresh，以便當前頁麵容器可以調用endPulldownToRefresh等方法
	...
	$.plusReady(function() {
		$.ajaxSettings = $.extend($.ajaxSettings, {
			xhr: function(settings) {
				if (settings.crossDomain) { //強製使用plus跨域
					return new plus.net.XMLHttpRequest();
				}
				//僅在webview的url為遠程文件，且ajax請求的資源不同源下使用plus.net.XMLHttpRequest
	...
	var Scroll = $.Class.extend({
		init: function(element, options) {
			...
			this.options = $.extend(true, {
				...
				snapX: 0.5, //橫向切換距離(以當前容器寬度為基準)
				snap: false, //圖片輪播，拖拽式選項卡

				bounce: true, //是否啟用回彈
				bounceTime: 500, //回彈動畫時間
				bounceEasing: ease.outCirc, //回彈動畫曲線

				scrollTime: 500,
				scrollEasing: ease.outCubic, //輪播動畫曲線

				directionLockThreshold: 5,

				parallaxElement: false, //視差元素
				parallaxRatio: 0.5
			}, options);
			...
			if (this.scroller) {
				this.refresh();
				//				if (this.options.startX !== 0 || this.options.startY !== 0) { //需要判斷嗎？後續根據實際情況再看看
		...
		_initEvent: function(detach) {
			...
			if (segmentedControl) { //靠，這個bug排查了一下午，阻止hash跳轉，一旦hash跳轉會導致可拖拽選項卡的tab不見
		...
		_drag: function(e) {
			...
			//			}
			var detail = e.detail;
			if (this.options.scrollY || detail.direction === 'up' || detail.direction === 'down') { //如果是豎向滾動或手勢方向是上或下
				//ios8 hack
				if ($.os.ios && parseFloat($.os.version) >= 8) { //多webview時，離開當前webview會導致後續touch事件不觸發
			...
			if (detail.direction === 'left' || detail.direction === 'right') {
				if (this.options.scrollX) {
					...
					if (!this.moved) { //識別角度(該角度導致輪播不靈敏)
						//						if (direction !== 'left' && direction !== 'right') {
						//							isReturn = true;
						//						} else {
						$.gestures.session.lockDirection = true; //鎖定方向
				...
				if (this.options.scrollY) {
					isPreventDefault = true;
					//					if (!this.moved) { //識別角度,豎向滾動似乎沒必要進行小角度驗證
			...
			if (this.moved || isPreventDefault) {
				e.stopPropagation(); //阻止冒泡(scroll類嵌套)
				detail.gesture && detail.gesture.preventDefault();
			}
			if (isReturn) { //禁止非法方向滾動
				return;
			}
			if (!this.moved) {
				$.trigger(this.scroller, 'scrollstart', this);
			} else {
				e.stopPropagation(); //move期間阻止冒泡(scroll嵌套)
			}
			var deltaX = 0;
			var deltaY = 0;
			if (!this.moved) { //start
		...
		_transitionTime: function(time) {
			...
			if (this.parallaxElement && this.options.scrollY) { //目前僅支持豎向視差效果
			...
			if (time) { //自定義timer，保證webkitTransitionEnd始終觸發
				this.transitionTimer && this.transitionTimer.cancel();
		...
		_reLayout: function() {
			...
			if (this.options.snap && typeof this.options.snap === 'string') {
				...
				this._initSnap(); //需要每次都_initSnap麼。其實init的時候執行一次，後續resize的時候執行一次就行了吧.先這麼做吧，如果影響性能，再調整
			}
		},
...
(function($, window, document, undefined) {
	...
	var PullRefresh = $.Scroll.extend($.extend({
		...
		pulldownLoading: function(y, time) {
			typeof y === 'undefined' && (y = this.options.down.height); //默認高度
	...
	$.fn.pullRefresh = function(options) {
		if (this.length === 1) {
			...
			if (options.down && options.down.auto) { //如果設置了auto，則自動下拉一次
				pullRefreshApi.pulldownLoading(options.down.autoY);
			} else if (options.up && options.up.auto) { //如果設置了auto，則自動上拉一次
...
(function($, window) {
	...
	var Slider = $.Slider = $.Scroll.extend({
		...
		handleEvent: function(e) {
			...
			switch (e.type) {
				...
				case $.eventName('shown', 'tab'):
					if (~this.snaps.indexOf(e.target)) { //避免嵌套監聽錯誤的tab show
		...
		_flick: function(e) {
			...
			//			}
			if (e.type === 'flick') {
				if (detail.deltaTime < 200) { //flick，太容易觸發，額外校驗一下deltaTime
		...
		_initSnap: function() {
			...
			if (!this.currentPage.x) {
				//當slider處於隱藏狀態時，導致snap計算是錯誤的，臨時先這麼判斷一下，後續要考慮解決所有scroll在隱藏狀態下初始化屬性不正確的問題
		...
		_gotoItem: function(slideNumber, time) {
			this.currentPage = this._getPage(slideNumber, true); //此處傳true。可保證程序切換時，動畫與人手操作一致(第一張，最後一張的切換動畫)
	...
	$.ready(function() {
		...
		//		}, 500); //臨時處理slider寬度計算不正確的問題(初步確認是scrollbar導致的)
 ...
 */
(function($, document) {
	if (!($.os.plus && $.os.android)) { //僅在android的5+版本使用
		return;
	}
	...
	var PlusPullRefresh = $.Class.extend({
			...
			self.scrollInterval = window.setInterval(function() {
				if (self.isScroll && !self.loading) {
					if (window.pageYOffset + window.innerHeight + 10 >= document.documentElement.scrollHeight) {
						self.isScroll = false; //放在這裏是因為快速滾動的話，有可能檢測時，還沒到底，所以隻要有滾動，沒到底之前一直檢測高度變化
			...
			if (self.topPocket && self.options.webviewId) {
				$.plusReady(function() {
					...
					webview.addEventListener("dragBounce", function(e) {
						...
						switch (e.status) {
							case "beforeChangeOffset": //下拉可刷新狀態
								self._setCaption(downOptions.contentdown);
								break;
							case "afterChangeOffset": //鬆開可刷新狀態
								self._setCaption(downOptions.contentover);
								break;
							case "dragEndAfterChangeOffset": //正在刷新狀態
								//執行下拉刷新所在webview的回調函數
		...
		handleEvent: function(e) {
			...
			//5+的plusscrollbottom當頁麵內容較少時，不觸發
		...
		setStopped: function(stopped) { //該方法是子頁麵調用的
			this.stopped = !!stopped;
			//TODO 此處需要設置當前webview的bounce為none,目前5+有BUG
			var webview = plus.webview.currentWebview();
			if (this.stopped) {
				webview.setStyle({
					bounce: 'none'
		...
		pulldownLoading: function() { //該方法是子頁麵調用的
			$.plusReady(function() {
		...
		//		_pulldownLoading: function() { //該方法是子頁麵調用的
		//			var self = this;
		//			$.plusReady(function() {
		...
		//			});
		//		},
		endPulldownToRefresh: function() { //該方法是子頁麵調用的
			var webview = plus.webview.currentWebview();
			webview.parent().evalJS("mui&&mui(document.querySelector('.mui-content')).pullRefresh('" + JSON.stringify({
				webviewId: webview.id
			}) + "')._endPulldownToRefresh()");
		},
		_endPulldownToRefresh: function() { //該方法是父頁麵調用的
			var self = this;
			if (self.topPocket && self.options.webview) {
				self.options.webview.endPullToRefresh(); //下拉刷新所在webview回彈
				self.loading = false;
				self._setCaption(self.options.down.contentdown, true);
				setTimeout(function() {
		...
		pullupLoading: function(callback) {
			...
			setTimeout(function() {
				...
				self.pullCaption.innerHTML = ''; //修正5+裏邊第一次加載時，文字顯示的bug(還會顯示出來個“多”,猜測應該是渲染問題導致的)
	...
	$.fn.pullRefresh = function(options) {
		...
		var pullRefreshApi = null;
		var attrWebviewId = options.webviewId && options.webviewId.replace(/\//g, "_"); //替換所有"/"
		var id = self.getAttribute('data-pullrefresh-plus-' + attrWebviewId);
		if (!id) { //避免重複初始化5+ pullrefresh
			id = ++$.uuid;
...
(function($, window, document, name) {
	...
	var OffCanvas = $.Class.extend({
		...
		handleEvent: function(e) {
			switch (e.type) {
				case $.EVENT_START:
					e.target && !this._preventDefaultException(e.target, this.options.preventDefaultException) && e.preventDefault();
					break;
				case 'webkitTransitionEnd': //有個bug需要處理，需要考慮假設沒有觸發webkitTransitionEnd的情況
					if (e.target === this.scroller) {
						this._dispatchEvent();
					}
					break;
					...
					if (this.isDragging) {
						...
						if (!this.slideIn) {
							...
							if (ratio === 0) {
								...
								this._dispatchEvent(); //此處不觸發webkitTransitionEnd,所以手動dispatch
								return;
							}
							if (direction === 'right' && ratio >= 0 && (ratio >= 0.5 || detail.swipe)) { //右滑打開
								this.openPercentage(100);
							} else if (direction === 'right' && ratio < 0 && (ratio > -0.5 || detail.swipe)) { //右滑關閉
								this.openPercentage(0);
							} else if (direction === 'right' && ratio > 0 && ratio < 0.5) { //右滑還原關閉
								this.openPercentage(0);
							} else if (direction === 'right' && ratio < 0.5) { //右滑還原打開
								this.openPercentage(-100);
							} else if (direction === 'left' && ratio <= 0 && (ratio <= -0.5 || detail.swipe)) { //左滑打開
								this.openPercentage(-100);
							} else if (direction === 'left' && ratio > 0 && (ratio <= 0.5 || detail.swipe)) { //左滑關閉
								this.openPercentage(0);
							} else if (direction === 'left' && ratio < 0 && ratio >= -0.5) { //左滑還原關閉
								this.openPercentage(0);
							} else if (direction === 'left' && ratio > 0.5) { //左滑還原打開
								this.openPercentage(100);
							} else { //默認關閉
								this.openPercentage(0);
							}
							if (ratio === 1 || ratio === -1) { //此處不觸發webkitTransitionEnd,所以手動dispatch
								this._dispatchEvent();
							}
						} else {
							if (x >= 0) {
							...
							if (direction === 'right' && ratio <= 0 && (ratio >= -0.5 || detail.swipe)) { //右滑打開
								...
								this.openPercentage(0);
							} else if (direction === 'right' && ratio <= -0.5) { //右滑還原關閉
								this.openPercentage(0);
							} else if (direction === 'right' && ratio > 0 && ratio <= 0.5) { //右滑還原打開
								this.openPercentage(-100);
							} else if (direction === 'left' && ratio >= 0 && (ratio <= 0.5 || detail.swipe)) { //左滑打開
								this.openPercentage(-100);
							} else if (direction === 'left' && ratio < 0 && (ratio <= -0.5 || detail.swipe)) { //左滑關閉
								this.openPercentage(0);
							} else if (direction === 'left' && ratio >= 0.5) { //左滑還原關閉
								this.openPercentage(0);
							} else if (direction === 'left' && ratio >= -0.5 && ratio < 0) { //左滑還原打開
								this.openPercentage(100);
							} else {
								this.openPercentage(0);
							}
		...
		initEvent: function() {
			...
			if (this.classList.contains('mui-draggable')) {
				this.wrapper.addEventListener($.EVENT_START, this); //臨時處理
	...
	window.addEventListener('tap', function(e) {
		...
		//TODO 此處類型的代碼後續考慮統一優化(target機製)，現在的實現費力不討好
		var target = e.target;
		for (; target && target !== document; target = target.parentNode) {
...
(function($, window, document, name) {
	...
	var backdrop = (function() {
		...
		element.addEventListener('tap', function(e) {
			...
			if (popover) {
				...
				document.body.setAttribute('style', ''); //webkitTransitionEnd有時候不觸發？
			}
		});
	...
	var togglePopover = function(popover, anchor, state) {
		...
		//remove一遍，以免來回快速切換，導致webkitTransitionEnd不觸發，無法remove
		...
		if (_popover) {
			...
			//			fixedPopoverScroll(false);
			//同一個彈出則直接返回，解決同一個popover的toggle
			if (popover === _popover) {
...
(function($, window, document, name, undefined) {
	...
	var handle = function(event, target) {
		if (target.classList && (target.classList.contains(CLASS_CONTROL_ITEM) || target.classList.contains(CLASS_TAB_ITEM))) {
			if (target.parentNode && target.parentNode.classList && target.parentNode.classList.contains(CLASS_SEGMENTED_CONTROL_VERTICAL)) {
				//vertical 如果preventDefault會導致無法滾動
			} else {
	...
	window.addEventListener('tap', function(e) {
		...
		for (var i = 0; i < _contents.length; i++) { //查找直屬子節點
...
(function($, window, name) {
	...
	Toggle.prototype.start = function(e) {
		...
		if (this.toggleWidth === 0 || this.handleWidth === 0) { //當switch處於隱藏狀態時，width為0，需要重新初始化
			this.init();
		}
	};
...
(function($, window, document) {
	...
	var handleEvent = {
		...
		drag: function(event) {
			...
			if (direction === 'left' && (angle > 150 || angle < -150)) {
				if (buttonsRight || (buttonsLeft && isOpened)) { //存在右側按鈕或存在左側按鈕且是已打開狀態
					isMoved = true;
				}
			} else if (direction === 'right' && (angle > -30 && angle < 30)) {
				if (buttonsLeft || (buttonsRight && isOpened)) { //存在左側按鈕或存在右側按鈕且是已打開狀態
					isMoved = true;
				}
			}
			if (isMoved) {
	...
	window.addEventListener($.EVENT_END, function(event) { //使用touchend來取消高亮，避免一次點擊既不觸發tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
	...
	window.addEventListener($.EVENT_CANCEL, function(event) { //使用touchcancel來取消高亮，避免一次點擊既不觸發tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
...
(function($, window, document) {
	...
	var createAlert = function(message, title, btnValue, callback, type) {
		...
		if (!$.os.plus || type === 'div') {
			return createPopup(createInner(message, title || '提示') + createButtons([btnValue || '確定']), callback);
		}
		return plus.nativeUI.alert(message, callback, title || '提示', btnValue || '確定');
	};
	var createConfirm = function(message, title, btnArray, callback, type) {
		...
		if (!$.os.plus || type === 'div') {
			return createPopup(createInner(message, title || '提示') + createButtons(btnArray || ['取消', '確認']), callback);
		}
		return plus.nativeUI.confirm(message, callback, title, btnArray || ['取消', '確認']);
	};
	var createPrompt = function(message, placeholder, title, btnArray, callback, type) {
		...
		if (!$.os.plus || type === 'div') {
			return createPopup(createInner(message, title || '提示', createInput(placeholder)) + createButtons(btnArray || ['取消', '確認']), callback);
		}
		return plus.nativeUI.prompt(message, callback, title || '提示', placeholder, btnArray || ['取消', '確認']);
	};
...
(function($, document) {
	...
	/**
	 * 創建並顯示進度條 
	 * @param {Object} container  可選，默認body，支持selector,DOM Node,mui wrapper
	 * @param {Object} progress 可選，undefined表示循環，數字表示具體進度
	 * @param {Object} color 可選，指定顏色樣式(目前暫未提供實際樣式，可暫時不暴露此參數)
	 */
	 ...
	 * 設置指定進度條進度 
	 * @param {Object} container  可選，默認body，支持selector,DOM Node,mui wrapper
	 * @param {Object} progress 可選，默認0 取值範圍[0-100]
	 * @param {Object} speed 進度條動畫時間
	 */
...
(function($, window, document) {
	...
	Input.prototype.initElementEvent = function() {
		...
		if (this.sliderActionClass) {
			...
			var timer = null;
			var showTip = function() { //每次重新計算是因為控件可能被隱藏，初始化時計算是不正確的
