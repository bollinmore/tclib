...
//			placeholder //默認圖片
			this.options = $.extend({
				selector: '',//查詢哪些元素需要lazyload
				diff: false,//距離視窗底部多少像素出發lazyload
				force: true,//強製加載(不論元素是否在是視窗內)
				autoDestroy: true,//元素加載完後是否自動銷毀當前插件對象
				duration: 70//滑動停止多久後開始加載
			}, options);
		...
		_initLoadFn: function() {
			var self = this;
			self._loadFn = this._buffer(function() { // 加載延遲項
		...
		/**
		 *根據加載函數實現加載器
		 *@param {Function} load 加載函數
		 *@returns {Function} 加載器
		 */
		...
		handle: function() {
			//throw new Error('需子類實現');
		},
		refresh: function(check) {
			if (check) { //檢查新的lazyload
				this.addElements();
			}
			this._loadFn();
		},
