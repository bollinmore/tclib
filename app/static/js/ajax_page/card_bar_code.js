mui.plusReady(function() {
	...
	function readyLoad() {
		try {
			var cardNumber = localStorage.getItem("barcode");
			if(cardNumber != null) {
				ca.showWaiting();
				var arrInfo = cardNumber.split("#");
				var date = new Date();
				var today = date.toLocaleDateString();
				$userdata.eq(0).html(arrInfo[2].replace("ok", ""));
				$userdata.eq(1).html(arrInfo[1]);
				$userdata.eq(2).html(today);
				$barcode.barcode(arrInfo[1], "code128", {
					output: "bmp",
					barWidth: 2,
